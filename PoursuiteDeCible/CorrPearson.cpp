...
double pearson(const CImg<double>& motif, const CImg<double>& sous_image) {
    double mu1 = motif.mean();
    double mu2 = sous_image.mean();
    double sigma1 = sqrt(motif.variance());
    double sigma2 = sqrt(sous_image.variance());
    ...
    for(int i = 0; i < nx; ++i) {
        for(int j = 0; j < ny; ++j) {
            ...
            corr += (val_motif - mu1) * (val_sous_image - mu2);
        }
    }
    corr /= (sigma1 * sigma2);
    corr /= (nx * ny);
...
int main(int argc, char *argv[]) {
    ...
    fs::remove_all("resultsPearson");
    fs::create_directory("resultsPearson");
    ...
    while (!Image_disp.is_closed() && !Image_disp.is_keyESC()) {
        ...
        if (Image_disp.button() & 1 && Image_disp.mouse_x() >= 0 && Image_disp.mouse_y() >= 0) {
            if (is_first_click) {
                ...
                Image_lue.save("resultsPearson/result000.png");
    ...
    for (size_t i = 1; i < images.size(); ++i) {
        ...
        double meilleure_correlation = std::numeric_limits<double>::min();
        
        int zone_recherche = 10;
        bool seuil_depasse = false;
        for(int x = std::max(0, meilleure_x - zone_recherche); x <= std::min(Image_lue.width() - nx, meilleure_x + zone_recherche) && !seuil_depasse; ++x) {
            for(int y = std::max(0, meilleure_y - zone_recherche); y <= std::min(Image_lue.height() - ny, meilleure_y + zone_recherche) && !seuil_depasse; ++y) {
                
                if (x + nx - 1 >= Image_lue.width() || y + ny - 1 >= Image_lue.height()) continue;
                
                CImg<double> sous_image = Image_lue.get_crop(x, y, x + nx - 1, y + ny - 1);

                double correlation = pearson(motif, sous_image);

                if(correlation > meilleure_correlation) {
                    meilleure_correlation = correlation;
                    meilleure_x = x;
                    meilleure_y = y;
                    if(meilleure_correlation > 0.7) {
                        seuil_depasse = true;;
                    }
                }
            }
        ...
        char filename[256];
        sprintf(filename, "resultsPearson/result%03d.png", (int)i);
        Image_lue.save(filename);
    }
    // Pour générer le pdf (plus facile pour tout visualiser avec mupdf)
    char command[256];
    sprintf(command, "magick resultsPearson/*.png resultsPearson/output.pdf");
