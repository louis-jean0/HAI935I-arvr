...
double ZSAD(const CImg<double>& motif, const CImg<double>& sous_image) {
    ...
    for(int i = 0; i < nx; ++i) {
        for(int j = 0; j < ny; ++j) {
            ...
            somme += std::abs((val_motif - mu1) - (val_sous_image - mu2));
        }
    }
    somme /= (sigma1 * sigma2);
    somme /= (nx * ny);
...
int main(int argc, char *argv[]) {
    ...
    fs::remove_all("resultsZSAD");
    fs::create_directory("resultsZSAD");
    ...
    while (!Image_disp.is_closed() && !Image_disp.is_keyESC()) {
        ...
        if (Image_disp.button() & 1 && Image_disp.mouse_x() >= 0 && Image_disp.mouse_y() >= 0) {
            if (is_first_click) {
                ...
                Image_lue.save("resultsZSAD/result000.png");
    ...
    for (size_t i = 1; i < images.size(); ++i) {
        ...
        for (int x = std::max(0, meilleure_x - zone_recherche); x <= std::min(Image_lue.width() - nx, meilleure_x + zone_recherche); ++x) {
            for (int y = std::max(0, meilleure_y - zone_recherche); y <= std::min(Image_lue.height() - ny, meilleure_y + zone_recherche); ++y) {
                ...
                double distance = ZSAD(motif, sous_image);
        ...
        char filename[256];
        sprintf(filename, "resultsZSAD/result%03d.png", (int)i);
        Image_lue.save(filename);
    }
    char command[256];
    sprintf(command, "magick resultsZSAD/*.png resultsZSAD/output.pdf");
